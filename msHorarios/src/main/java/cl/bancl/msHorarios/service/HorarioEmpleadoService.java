...
public class HorarioEmpleadoService {
    ...
    public HorarioEmpleadoDTO obtenerHorarioConEmpleado(Integer empleadoId) {
        ...
        List<DiasTrabajoDTO> diasDTO = listaLocal.stream().map(h -> {  //el steam  es un for pero más eficiente para transformar listas, en este caso de HorariosEmpleado a DiasTrabajoDTO
            DiasTrabajoDTO dto = new DiasTrabajoDTO();                 //map es un for pero para transformar objetos, en este caso de HorariosEmpleado a DiasTrabajoDTO
            dto.setDiaSemana(h.getDiaSemana());
            dto.setHoraInicio(h.getHoraInicio());
            dto.setHoraFin(h.getHoraFin());
            return dto;
        }).collect(Collectors.toList()); //collect(Collectors.toList()) es para convertir el stream de vuelta a una lista 

        // Juntamos todo en la respuesta final
        HorarioEmpleadoDTO respuesta = new HorarioEmpleadoDTO();
        respuesta.setEmpleadoId(empleadoId);
        // Suponiendo que tu EjecutivosDTO tiene getNombre() y getApellido()
    ...
    // metodo para desactivar un horario por licencia, recibe el id del horario y el motivo de la licencia, luego actualiza el campo activo a false y guarda la observación en el campo observaciones
...
public List<HorariosEmpleado> buscarPorEmpleado(Integer empleadoId) {
    ...
    // 4. Retornamos la lista ya procesada con los nombres puestos
    return horarios;
}
